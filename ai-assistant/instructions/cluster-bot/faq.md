...
1. **What is `workflow_launch`?**
   ...
   The parameter can then be treated as a bash array of the nested parameters (see [example in openshift/release](https://github.com/openshift/release/blob/7c9e17caab51f23303d648a5f2f491f770c72c35/ci-operator/step-registry/baremetalds/devscripts/setup/baremetalds-devscripts-setup-commands.sh#L167-L178)).
   ...
   The list of supported platforms can be found in the ci-chat-bot's code (see `SupportedPlatforms` in [pkg/manager/prow.go](https://github.com/openshift/ci-chat-bot/blob/master/pkg/manager/prow.go)).

   The workflow config file also supports setting `base_images` for workflows that may require special base images to run, as well as an `architecture` field to configure the CPU architecture. Supported architectures are `amd64` (default), `arm64`, and `multi`.
...
2. **How can I make a workflow runnable by `workflow-*` commands**
   ...
   1. Make a PR editing the
      ...
      exists that can be used to configure the architecture for the cluster
      (supported: `amd64`, `arm64`, `multi`).
...
2. In order to access the OpenShift Console of the cluster you can use a proxy switcher extension e.g. [Proxy Switcher and Manager](https://chromewebstore.google.com/detail/proxy-switcher-and-manage/onnfghpihccifgojkpnnncpagjcdbjod?hl=en), and manually configure the Proxy IP address and Port from the kubeconfig's `proxy-url` field in the extension's settings.
